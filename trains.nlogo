...
patches-own [ taken? walkway? chair? window-seat? ]
...
to setup
  clear-all
  set-default-shape chairs "chairshape"
  set-default-shape persons "personshape"
  ask patches [ set chair? false ]
  place-chairs
  set-window-seats
  enter-coupe
...
to move-people
  ask persons with [seated? = false] [
    ...
    ifelse can-sit? [
      ...
      ; Not window seat
      [
        let windowseats ([neighbors4] of patch-here) with [window-seat? = true]
        if any? windowseats [
          let target one-of windowseats
...
to-report is-window-seat?
  report [window-seat?] of patch-here
end

to-report can-sit?
  ifelse [chair?] of patch-here
  [
    report not [taken?] of patch-here
  ]
  [report false ]
end
...
to place-chairs
  ...
  repeat 2 [
    ...
    [
      set x 0
      repeat 21 [
        ask patch x y [ set walkway? true]
        set x x + 1
      ]
    ]
    set y y + 1
  ...
  ;ask patches with [walkway? = true] [
  ;  set pcolor green
  ;]
end

to set-window-seats
  ask patches with [chair? = true and (pycor = 2 or pycor = 6)]
  [ set window-seat? true ]
end
...
to place-four [ x y ]
  make-chair x y true
  make-chair x y + 1 true
  make-chair x + 2 y false
  make-chair x + 2 y + 1 false
end


to make-chair [ x y h ]
  create-chairs 1 [
    set heading ifelse-value h [90] [-90]
    move-to patch x y
    set window-seat? false
    set chair? true
    set taken? false
  ]
end
...
to make-people
  create-persons 1 [
    move-to one-of patches with [ any? chairs-here and not taken?]
    set taken? true
  ]
end
...
true
0
Rectangle -13345367 true false 0 240 300 300
Rectangle -13345367 true false 0 30 45 285
Rectangle -13345367 true false 255 30 315 270
Rectangle -13791810 true false 45 180 255 240
Rectangle -13345367 true false 0 240 330 300
